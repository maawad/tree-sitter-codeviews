...
from ...utils import hip_nodes
from loguru import logger

class CFGGraph_hip(CFGGraph):
    def __init__(self, src_language, src_code, properties, root_node, parser):
        ...
        self.node_list = None
        self.statement_types = hip_nodes.statement_types
        self.CFG_node_list = []
        self.CFG_edge_list = []
        self.records = {
            "basic_blocks": {},
        ...
        self.CFG_node_list, self.CFG_edge_list = self.CFG_hip()
    ...
    def CFG_hip(self):
        ...
        _, self.node_list, self.CFG_node_list, self.records = hip_nodes.get_nodes(
        ...
        for node_key, node_value in node_list.items():
            ...
            if current_node_type in self.statement_types["non_control_statement"]:
                ...
                if (
                    hip_nodes.return_switch_child(node_value) is None
        ...
        for node_key, node_value in node_list.items():
            ...
            # ------------------------------------------------------------------------------------------------
                ...
                switch_parent = hip_nodes.return_switch_parent_statement(node_value, self.statement_types["non_control_statement"])
